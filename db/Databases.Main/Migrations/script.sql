...
CREATE TABLE infrastructure.es_instances (
    ...
    CONSTRAINT "FK_es_instances_locations_location_name" FOREIGN KEY (location_name) REFERENCES infrastructure.locations (name) ON DELETE CASCADE
);
...
CREATE TABLE history.end_of_day_balances (
    ...
    event_id bigint NOT NULL,
    value numeric NOT NULL,
    CONSTRAINT "PK_end_of_day_balances" PRIMARY KEY (dt, account_id, currency_id),
    ...
    CONSTRAINT "FK_end_of_day_balances_as_instances_account_service_name" FOREIGN KEY (account_service_name) REFERENCES infrastructure.as_instances (name) ON DELETE RESTRICT,
    ...
    CONSTRAINT "FK_end_of_day_balances_events_account_service_name_event_id" FOREIGN KEY (account_service_name, event_id) REFERENCES events.events (account_service_name, event_id) ON DELETE CASCADE
);
...
CREATE INDEX "IX_end_of_day_balances_account_service_name_event_id" ON history.end_of_day_balances (account_service_name, event_id);
...
VALUES ('20260330134321_Initial', '9.0.8');
