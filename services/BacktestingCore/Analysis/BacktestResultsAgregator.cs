    ...
    IEventHandler<AccountEndOfDayEvt>
{
    ...
    private readonly Dictionary<int, List<SharePriceHistory>> _returns;
    public IReadOnlyDictionary<int, IReadOnlyList<SharePriceHistory>> Returns => 
        _returns.ToDictionary(kv => kv.Key, kv => (IReadOnlyList<SharePriceHistory>)kv.Value);
    ...
    public BacktestResultsAgregator(TestExecutorOptions options, int numberOfStrategies)
    {
        ...
        if (options.RecordSharePrices) _returns = new (numberOfStrategies);
